...
**Verde entra, rojo sale, amarillo se guarda.** Antes el color era decorativo: cada categoría sacaba el suyo de una paleta rotatoria (`PALETTE`), así que "Gastos recurrentes" podía salir verde en el dashboard mientras "Ahorros" salía rojo —exactamente al revés de lo que significan—. Ahora `engine/semantica.js` decide: los roles `cor` y `lar` son ahorro, el resto de categorías son gasto, las metas son siempre ahorro, y un movimiento es ingreso, ahorro (si va a una meta) o gasto. Cada significado tiene un tono que manda, y ese es el que sale en las barras, los puntos y los montos: sin aclarar, para que no haya escalas.

**El donut es la excepción, y por eso no usa esos tres colores.** Repartir un mismo rojo en cuatro claridades dejaba la gráfica ilegible: todas las porciones se parecían y el ojo no separaba una categoría de otra. Ahí el color no tiene que decir qué significa la plata —el título ya dice que todo eso es gasto—, tiene que decir cuál porción es cuál. Las porciones y sus puntos en la lista de Análisis usan `colorCategoria(i)`, diez hues distintos (`--cat-1`…`--cat-10` en `tokens.css`), asignados por el puesto de la categoría en la lista, así que el color de una porción no cambia mientras no cambie el orden.
